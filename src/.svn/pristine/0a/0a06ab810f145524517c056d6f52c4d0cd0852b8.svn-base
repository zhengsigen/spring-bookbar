...
import com.c1801.spring.dzy.service.*;
...
public class UserWXLoginController {
    ...
    public ResData getCode(@RequestParam("code") String code, @RequestParam(value = "state", required = false) String state, HttpSession session) throws UnsupportedEncodingException {
        ...
        if (state != null && state.length() == 33 && state.endsWith("@")) {
    ...
    //新增用户
    @PostMapping
    public ResData addUser(HttpSession session) throws IOException {
        logger.info("新用户新增");
        ...
        User user = wxUserService.queryUserByOpenId(addUser.getOpenId());
        if (resData.getData() != null) {
            session.setAttribute("user", resData.getData());
        }
        //如果inviteCode！=null说明是被邀请注册的用户
        String inviteCode = (String) session.getAttribute("inviteCode");
        if (inviteCode != null) {
            //发送优惠券
            couponService.sendUserCoupon(user.getId(),"");
            inviteService.createInviteRecord(user.getId(), inviteCode);
            //消息推送
            User inviterUser = inviteRecordMapper.queryInviteRecordByInviteeId(user.getId());
            UserSubscribe userSubscribe = userSubscribeMapper.getUserSubscribe(inviterUser.getId());
            if (userSubscribe.getInvitation()) {
                messageTemplateService.sendInvitationResult(inviterUser, user, "接收邀请");
            }
        }
        //新增一个订阅用户
...
//       }
        ...
        User tempUser = wxUserService.queryUserByOpenId(user.getOpenId());
        if (resData.getData() != null) {
            session.setAttribute("user", resData.getData());
        }
        //如果inviteCode！=null说明是被邀请注册的用户
        ...
        if (inviteCode != null) {
            //发送优惠券
            couponService.sendUserCoupon(tempUser.getId(),"");
            inviteService.createInviteRecord(tempUser.getId(), inviteCode);
            //消息推送
            User inviterUser = inviteRecordMapper.queryInviteRecordByInviteeId(tempUser.getId());
            UserSubscribe userSubscribe = userSubscribeMapper.getUserSubscribe(inviterUser.getId());
            if (userSubscribe.getInvitation()) {
                messageTemplateService.sendInvitationResult(inviterUser, tempUser, "接收邀请");
            }
        }
        return resData;
    }
