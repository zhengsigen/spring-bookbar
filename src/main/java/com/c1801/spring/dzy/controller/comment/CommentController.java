...
public class CommentController {
    ...
    public ResData delComment(@RequestParam("commentId") Integer commentId, HttpSession session) {
        ...
        commentApplaudMapper.deleteApplaud(commentId);
     ...
     */
    @GetMapping
    public ResData queryCommentList(@RequestParam("bookId")Integer bookId, @RequestParam(value = "pageSize", defaultValue = "5")Integer pageSize, @RequestParam(value = "page", defaultValue = "1")Integer page,  HttpSession session){
        ...
        if(bookComment == null){
            return ResData.ofFail(3030, "当前书籍没有任何人评论", null);
        }
        //分页
     ...
     */
    @PostMapping("/message")
    public ResData addReplyOfComments(@RequestParam("commentId")Integer commentId,@RequestParam("message")String message,HttpSession session) throws IOException {
        User user = (User) session.getAttribute("user");
        if(user == null){
            user = new User();
            return ResData.ofFail(2002, "用户没有登录", null);
        ...
        if(userSubscribe.getAgree()){
            User receiveUser = userMapper.getUserById(comment.getUserId());
            messageTemplateService.sendDynamic(receiveUser, user, message,comment.getBookId(),commentId);
        }
