...
public class WXUserService {
    ...
    public Map<String, Object> preparingData(Integer orderId, String tradeType, Long fee) {
        ...
        //订单结束时间
        long time_expire = order.getCreateTime().getTime() + orderPay.getAutoConfirmTime();
        //如果订单剩余时间小于一分钟
        if ((time_expire - new Date().getTime()) < (1000 * 60)) return null;
        String tempTime = format.format(time_expire);
        tempTime = tempTime.replaceAll("-", "");
        tempTime = tempTime.replaceAll(":", "");
        tempTime = tempTime.replaceAll(" ", "");
        System.out.println(tempTime);
        params.put("time_expire", tempTime);
        ...
        params.put("notify_url", "https://wx.panqingshan.cn/notice/4/dzy/order/pay/notice");
        //签名
        return params;
    }
    ...
    public ResData createQR(Integer orderId, Long fee) throws IOException {
        ...
        if (params == null) return ResData.ofFail(2006, "订单已经过期");
